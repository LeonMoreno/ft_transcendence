...
function loginUser(username, password) {
  ...
  .then(response => {
    if (response.ok) {
      return response.json(); // If login is successful, parse the JSON response
    } else {
      response.json().then(data => {
        // If the response is not ok, try to parse the response body as JSON to get error details
        console.error('Login failed with response:', data);
        showNotification(`Login failed: ${data.detail}`, "error"); // Show more detailed error message
      });
      throw new Error('Login failed');
    }
  })
  .then(data => {
    console.log(data.access); // Correctly log the access token
    localStorage.setItem('jwt', data.access); // Store the JWT token in localStorage
    setTimeout(() => {
      window.location.href = '/#'; // Redirect after a short delay
    }, 500);
  })
  .catch((error) => {
    console.error('Error:', error); // Log any errors that occur during the fetch or JSON parsing
  });
}
